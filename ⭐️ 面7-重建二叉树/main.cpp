 ...
 )
{
    ...
    //中序找到根节点
    int *finderInorder = startInorder;
    while (finderInorder <= endInorder && *finderInorder != root->m_nValue)
    {
        finderInorder ++;
    }
    
    int leftLength = (int)(finderInorder - startInorder);
    if (leftLength > 0) {
        //构建左子树
        root->m_pLeft =  ConstructCore(startPreorder + 1,
                                       startPreorder + leftLength,
                                       startInorder,
                                       finderInorder - 1);
    }
    else
    {
        //构建右子树
        root->m_pRight = ConstructCore(startPreorder + leftLength + 1,
                                       endPreorder,
                                       finderInorder + 1,
                                       endInorder);
    }
...
//        throw;
//
//    int leftLength = rootInorder - startInorder;    //左子树规模
